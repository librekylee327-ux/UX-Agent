    ...
    <div className="space-y-6">
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-slate-700">도메인 프레임워크</h3>
          <div className="flex gap-2">
            <button
              onClick={addDimension}
              className="text-xs text-purple-600 hover:text-purple-700 border border-purple-200 px-3 py-1.5 rounded-lg transition-colors"
            >+ 차원 추가</button>
            <button
              onClick={save}
              disabled={saving}
        ...
        {dimensions.length === 0 ? (
          <div className="text-center py-8 border-2 border-dashed border-slate-200 rounded-xl">
            <p className="text-sm text-slate-400">차원을 추가해 도메인 프레임워크를 설계하세요</p>
            <p className="text-xs text-slate-300 mt-1">예: 경험 → 인지 → 행동 → 맥락</p>
          </div>
        ) : (
          <div className="space-y-4">
            {dimensions.map((dim, di) => (
              <div key={dim.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                <div className="flex items-center gap-2 p-3 bg-purple-50 border-b border-slate-200">
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent text-sm font-medium text-slate-800 placeholder-slate-300 focus:outline-none"
                  />
                  <button onClick={() => addSubDimension(dim.id)} className="text-xs text-slate-400 hover:text-purple-600">+ 세부</button>
                  <button onClick={() => removeDimension(dim.id)} className="text-slate-300 hover:text-rose-500 text-sm">×</button>
                </div>
                ...
                <div className="p-3 space-y-3">
                    ...
                    <div key={sub.id} className="pl-3 border-l-2 border-purple-200">
                      <div className="flex items-center gap-2 mb-2">
                        <input
                          ...
                          className="flex-1 bg-transparent text-xs font-medium text-slate-700 placeholder-slate-300 focus:outline-none"
                        />
                        <button onClick={() => addElement(dim.id, sub.id)} className="text-xs text-slate-400 hover:text-purple-600">+ 요소</button>
                        <button onClick={() => removeSubDimension(dim.id, sub.id)} className="text-slate-300 hover:text-rose-500 text-xs">×</button>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {sub.elements.map((el, ei) => (
                          <div key={ei} className="flex items-center gap-1 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1">
                            <input
                              type="text"
                              value={el}
                              onChange={(e) => updateElement(dim.id, sub.id, ei, e.target.value)}
                              placeholder="요소"
                              className="bg-transparent text-xs text-slate-700 placeholder-slate-300 focus:outline-none w-20"
                            />
                            <button onClick={() => removeElement(dim.id, sub.id, ei)} className="text-slate-300 hover:text-rose-500 text-xs">×</button>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {dim.sub_dimensions.length === 0 && (
                    <p className="text-xs text-slate-300 pl-2">세부 차원을 추가하세요</p>
                  )}
                </div>
              </div>
            ))}
      ...
      {/* Notes */}
      <section>
        <h3 className="text-sm font-semibold text-slate-700 mb-2">프레임워크 설계 메모</h3>
        <textarea
          value={notes}
          ...
          className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-purple-400 resize-none"
        />
      </section>
    </div>
  );
